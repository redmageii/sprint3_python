medicamentos={
    'Paracetamol 500mg':{'quantidade': 50, 'dosagem':'500mg','peso_unitario(mg)':800, 'classe': 'analgésico', 'localização': 'A1'},
    'Dipirona 1g':{'quantidade': 27, 'dosagem':'1g','peso_unitario(mg)':1200, 'classe': 'analgésico', 'localização': 'A2'},
    'Ciprofloxacino 500mg': {'quantidade': 6, 'dosagem':'500mg','peso_unitario(mg)':700, 'classe': 'antibiótico', 'localização': 'B1'}
}

classes=['antibiótico', 'analgésico', 'antipirético', 'antidepressivo', 'antialérgico', 'antivirótico', 'antidiabético', 'cardiovascular']
...
def cadastrar_item():
    while True:
        print('Nome do medicamento deve seguir a convenção:\nNOME SINGULAR + DOSAGEM (em ml ou mg, sem separação da unidade por espaço)')
        medicamento=input('Insira o nome do medicamento: ').capitalize()
...
def remover_item():
    if len(medicamentos.keys())>0:
        ...
        item=forca_opcao(medicamentos.keys(), 'Qual medicamento deseja remover? (especificar o nome seguido de dosagem)', 'Nome inválido')
...
def alterar_item():
    print(f'============================\n{'\n'.join(medicamentos.keys())}')
    item=forca_opcao(medicamentos.keys(), 'Qual medicamento deseja alterar? (especificar o nome seguido de dosagem)', 'Nome inválido')
